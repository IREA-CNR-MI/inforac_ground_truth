library(magrittr)
# import csv file ----
unepMapDocs_pdf <- vroom::vroom("listOfDocuments_complete_II.csv", ",", escape_double = FALSE, trim_ws = TRUE) %>%
  dplyr::select(c(1:7)) %>%
...
pad_it <- function(x){
  paste0("UNEP_", stringr::str_pad(x, npad, "left", "0"), "_II")
}
...
for (i in unepMapDocs_pdf$pdf_id) {
  ...
  tryCatch(
    download.file(
      ...
      destfile = paste0("PDFs_II/", file_name),
      mode = "wb",
      quiet = FALSE
    ),
    error = function(e) {
...
# store the results ----
saveRDS(unepMapDocs_pdf, file = "unepMapDocs_pdf_II.rds")
...
saveRDS(unepMapDocs_pdf, file = "unepMapDocs_pdf_final_II.rds")
unepMapDocs_pdf <- readRDS(file = "unepMapDocs_pdf_final_II.rds")
utils::write.csv(unepMapDocs_pdf, "unepMapDocs_pdf_final_II.csv", row.names = FALSE)
